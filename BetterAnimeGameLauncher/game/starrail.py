...
from BetterAnimeGameLauncher.component.jadeite import initJadeite
...
def launchHSR():
    # check config
    if "installed_hsr_ver" not in CONFIG:
        CONFIG["installed_hsr_ver"] = "0.0.0"
    if "lang" not in CONFIG:
        CONFIG["lang"] = ["en-us"]

    api = base64.b64decode("aHR0cHM6Ly9oa3JwZy1sYXVuY2hlci1zdGF0aWMuaG95b3ZlcnNlLmNvbS9oa3JwZ19nbG9iYWwvbWRrL2xhdW5jaGVyL2FwaS9yZXNvdXJjZT9jaGFubmVsX2lkPTEma2V5PXZwbE9WWDhWbjdjd0c4eWImbGF1bmNoZXJfaWQ9MzU=")
    ...
    if (CONFIG["installed_hsr_ver"] != game_version) or not os.path.exists(PATH_DATA_GAME_STARRAIL_DIR):
        print("Installing Honkai Star Rail...")
        installHSR(api_responce)
        saveCache()
        saveConfig()
    ...
    game_path = os.path.join(PATH_DATA_GAME_STARRAIL_DIR, game_executable)
...
def installHSR(api_responce):
    ...
    if not os.path.exists(PATH_DATA_GAME_STARRAIL_DIR):
        os.makedirs(PATH_DATA_GAME_STARRAIL_DIR)

    # check if it can just be upgraded instead redownloading the full game
    if(CONFIG["installed_hsr_ver"] != "0.0.0"):
        for diff in api_responce["data"]["game"]["diffs"]:
            if(diff["version"] == CONFIG["installed_hsr_ver"]):
                filename = os.path.basename(segment["path"])
                game_basefilename = filename #os.path.splitext(filename)[0]
    ...
    if not has_upgrade:
        ...
        filename = os.path.basename(api_responce["data"]["game"]["latest"]["path"])
        game_basefilename = filename#os.path.splitext(filename)[0]
        location = os.path.join(PATH_DATA_GAME_STARRAIL_DIR, filename)

        if not (os.path.exists(location)): #or (hashlib.md5(open(location,'rb').read()).hexdigest() != segment["md5"])
            download_obj = {}
            download_obj["url"] = api_responce["data"]["game"]["latest"]["path"]
            download_obj["md5"] = api_responce["data"]["game"]["latest"]["md5"]
    ...
    location = os.path.join(PATH_DATA_GAME_STARRAIL_DIR, game_basefilename)
    parts = getZipParts(location)

    if len(parts) > 0:
        with open(location, "ab") as outfile:
            for part_filename in parts:
                part_location = os.path.join(PATH_DATA_GAME_STARRAIL_DIR, part_filename)
    ...
    with zipfile.ZipFile(location, 'r') as zip_ref:
        zip_ref.extractall(PATH_DATA_GAME_STARRAIL_DIR)

    CONFIG["installed_hsr_ver"] = base_game_version
...
def getZipParts(zip_file):
    ...
    for num in range(1,999):
        ...
        path = os.path.join(PATH_DATA_GAME_STARRAIL_DIR, part_file)
